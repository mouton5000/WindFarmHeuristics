...
/*
 * JGraphT : a free Java graphTheory.graph-theory library Project Info:
...
public class FibonacciHeap<T> {
	 ...
	 */
	public FibonacciHeapNode<T> insert(T data, double key) {
	 ...
	 * -------------------------- (C) Copyright 1999-2007, by Nathan Fiedler and
	 * Contributors. Original Author: Nathan Fiedler Contributor(s): John V.
	 * Sichi $Id: FibonacciHeapNode.java 568 2007-09-30 00:12:18Z perfecthash $
	 * Changes ------- 03-Sept-2003 : Adapted from Nathan Fiedler (JVS); Name
	 * Date Description ---- ---- ----------- nf 08/31/97 Initial version nf
	 * 09/07/97 Removed FibHeapData interface nf 01/20/01 Added synchronization
	 * nf 01/21/01 Made Node an inner class nf 01/05/02 Added clear(), renamed
	 * empty() to isEmpty(), and renamed printHeap() to toString() nf 01/06/02
	 * Removed all synchronization JVS 06/24/06 Generics
	 */
	 ...
	 * @author Nathan Fiedler
	 */
	public class FibonacciHeapNode<U> {
		//~ Instance fields --------------------------------------------------------
		 ...
		 * first child node
		 */
		FibonacciHeapNode<U> child;

		/**
		 * left sibling node
		 */
		FibonacciHeapNode<U> left;

		/**
		 * parent node
		 */
		FibonacciHeapNode<U> parent;

		/**
		 * right sibling node
		 */
		FibonacciHeapNode<U> right;
		 ...
		 */
		public FibonacciHeapNode(U data, double key) {
			right = this;
			left = this;
			this.data = data;
			this.key = key;
		...
		public LinkedList<FibonacciHeapNode<U>> children() {
			LinkedList<FibonacciHeapNode<U>> l = new LinkedList<FibonacciHeapNode<U>>();
			l.add(this);
			FibonacciHeapNode<U> y, x = this.child;
			y = x;
			if (x != null) {
				do {
					l.addAll(x.children());
