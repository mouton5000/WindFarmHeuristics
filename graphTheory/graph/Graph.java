...
import graphTheory.graphDrawer.symbols.*;
...
public class Graph implements Parametable {
	...
	public ArrayList<Arc> getEdges() {
		ArrayList<Arc> ar = edges.stream().filter(a -> !this.hasVirtuallyRemoved(a)).collect(Collectors.toCollection(ArrayList::new));
		return ar;
	}
	...
	public LinkedList<Arc> getLinkedListEdges() {
		...
		ar.addAll(edges.stream().filter(a -> !this.hasVirtuallyRemoved(a)).collect(Collectors.toList()));
		return ar;
	}
	...
	/**
	 * @return An iterator over the set of nodes in this graph.
	 */
	public Iterator<Integer> getVerticesIterator() {
		return new PartialIterator<Integer>(vertices,
				n -> !Graph.this.hasVirtuallyRemoved(n));
	...
	public Iterator<Arc> getEdgesIterator() {
		return new PartialIterator<Arc>(edges, a -> !Graph.this.hasVirtuallyRemoved(a));
	 ...
	 *         belong to this graphTheory.graph, or is virtually removed.
	 */
	 ...
	 *         graphTheory.graph, or is virtually removed.
	 */
	...
	private boolean fillWithInputArcs(Integer n, Collection<Arc> col) {
		...
		if (l == null)
			return false;
		col.addAll(l.stream().filter(a -> !this.hasVirtuallyRemoved(a)).collect(Collectors.toList()));
		return true;
	}
	 ...
	 *         n does not belong to this graphTheory.graph, or is virtually removed.
	 */
	 ...
	 *         does not belong to this graphTheory.graph, or is virtually removed.
	 */
	...
	/**
	 * @param n
	 * @return an iterator over the arcs entering n in this null if n does
	...
	public Iterator<Arc> getInputArcsIterator(Integer n) {
		...
		if (l == null)
			return null;
		return new PartialIterator<Arc>(l, a -> !Graph.this.hasVirtuallyRemoved(a));
	}
	...
	private boolean fillWithInputNodes(Integer n, Collection<Integer> col) {
		...
		if (l == null)
			return false;
		col.addAll(l.stream().filter(a -> !this.hasVirtuallyRemoved(a)).map(Arc::getInput).collect(Collectors.toList()));
		return true;
	}
	...
	/**
	 * @param n
	 * @return an iterator over the arcs outgoing from n in this, null if n
	 *         does not belong to this graph, or is virtually removed.
	 */
	...
	private boolean fillWithOutputNodes(Integer n, Collection<Integer> col) {
		...
		if (l == null)
			return false;
		col.addAll(l.stream().filter(a -> !this.hasVirtuallyRemoved(a)).map(Arc::getOutput).collect(Collectors.toList()));
		return true;
	}
	...
	/**
	 * @param n
	 * @return an iterator over the undirected edges linked to n in this,
	 *         null if n does not belong to this graph, or is virtually removed.
	 */
	...
	private boolean fillWithUndirectedNeighbourNodes(Integer n,
		...
		if (l == null)
			return false;
		col.addAll(l.stream().filter(a -> !this.hasVirtuallyRemoved(a)).map(a -> this.getNeighbourNode(n, a)).collect(Collectors.toList()));
		return true;
	}
	...
	public Set<Integer> getAllNeighbourNodes(Integer n) {
		HashSet<Integer> h = getAllNeighbourEdges(n).stream().map(a -> this.getNeighbourNode(n, a)).collect(Collectors.toCollection(HashSet::new));
		return h;
	}
	...
	public boolean removeVertice(Integer node) {
		...
		this.getAllNeighbourEdges(node).forEach(this::removeEdge);
	...
	public boolean virtuallyRemoveVertice(Integer n) {
		return !(n == null || !this.contains(n)) && vrmVertices.add(n);
	}
	...
	public boolean cancelVirtuallyRemoveVertice(Integer n) {
		return n != null && vrmVertices.remove(n);
	}
	...
	public boolean virtuallyRemoveEdge(Arc a) {
		return !(a == null || !this.contains(a)) && vrmEdges.add(a);
	}
	...
	public boolean cancelVirtuallyRemoveEdge(Arc a) {
		return !(a == null || this.hasVirtuallyRemoved(a.getInput())
				|| this.hasVirtuallyRemoved(a.getOutput())) && vrmEdges.remove(a);
	}
	...
	public ArrayList<ArrayList<Integer>> getNodeLayers(Integer n1, Integer n2,
		...
		return getNodeLayers(n1, n2,
				c -> !checkOrientation || !c.second.isDirected()
	...
	public ArrayList<ArrayList<Integer>> getNodeLayersWithSpecificArcs(
		...
		return getNodeLayers(n1, n2,
				c -> (isDirected && c.second.isDirected() && c.first
	...
	public ArrayList<Integer> getDirectedCycleFrom(Integer v) {
		...
		TreeSet<Integer> l = new TreeSet<Integer>((v1, v2) -> {
